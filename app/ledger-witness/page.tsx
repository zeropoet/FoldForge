...
export default function LedgerWitness() {
  ...
  return <main className="min-h-screen bg-white text-black">
    ...
    <div className="mx-auto max-w-[1600px] px-5 py-10 md:px-8 md:py-16">
      <section className="grid gap-10 border-b border-black/20 pb-12 lg:grid-cols-[1fr_0.75fr] lg:items-end"><div><p className="text-[9px] uppercase tracking-[0.25em] text-black/40">FoldPortrait issuance / evidence / signature / relation</p><h1 className="mt-5 text-5xl font-light tracking-[-0.055em] md:text-7xl">Ledger Witness</h1><p className="mt-7 max-w-2xl text-sm leading-7 text-black/55">FoldPortrait’s exclusive XRPL issuance channel. Prepare one sealed work, hand its exact mint transaction to the configured Xaman wallet, and return the verified ledger result to its FoldPortrait record and Sovereign Standard vessel relation.</p></div><div className="border border-black/20 p-5 font-mono text-[8px] uppercase leading-5 tracking-[0.14em] text-black/35">FoldPortrait works only<br />Local source snapshots and drafts<br />XRPL mainnet / NFTokenMint<br />Human Xaman signature required</div></section>
      <section className="mt-12 grid gap-px bg-black/20 lg:grid-cols-[1fr_330px]">
        <div className="space-y-10 bg-white p-6 md:p-8"><div><p className="text-[9px] uppercase tracking-[0.22em] text-black/40">01 / Select admitted FoldPortrait work</p><label className="mt-5 block border border-black/25 p-4 text-[8px] uppercase tracking-[0.16em] text-black/45">Ready to mint / {actionableWorks.length.toString().padStart(2, "0")}<select className="mt-3 w-full bg-white text-sm normal-case text-black" value={preparedId} onChange={(event) => choosePrepared(event.target.value)}><option value="">{actionableWorks.length ? "Select prepared work" : "No prepared works have an available vessel"}</option>{actionableWorks.map((work) => <option key={work.artifact_id} value={work.artifact_id}>{work.sequence ? `${work.sequence}. ` : ""}{work.title} → Vessel {work.sequence ? units[work.sequence - 1]?.id : ""}</option>)}</select></label><p className="mt-3 font-mono text-[8px] uppercase leading-5 tracking-[0.13em] text-black/30">Shows only unminted catalog works whose claim-order Sovereign Standard vessel is already available.</p></div>
          <div><p className="text-[9px] uppercase tracking-[0.22em] text-black/40">02 / Verify and attach</p><div className="mt-5 grid gap-4"><input readOnly className="border-b border-black/25 bg-white py-3 text-xl outline-none" placeholder="Catalog work title" value={title} /><textarea readOnly className="min-h-24 border border-black/20 bg-white p-4 text-sm leading-6 outline-none" placeholder="Catalog description" value={description} /><input readOnly className="border-b border-black/25 bg-white py-3 font-mono text-[10px] outline-none" placeholder="Canonical FoldPortrait metadata URI" value={metadataUri} /><div className="border border-black/20 p-4"><p className="text-[8px] uppercase tracking-[0.16em] text-black/40">SS vessel relation / claim order</p><p className="mt-3 text-sm">{visibleUnits.length ? `Vessel ${visibleUnits[0]} / fixed automatically` : "Awaiting the corresponding claimed vessel"}</p></div></div></div>
          <div><p className="text-[9px] uppercase tracking-[0.22em] text-black/40">03 / Prepare and sign</p><div className="mt-5 flex flex-wrap gap-2"><button className="border border-black px-5 py-4 text-[8px] uppercase tracking-[0.18em]" onClick={prepare}>Prepare mint intent</button><button className="border border-black/35 px-5 py-4 text-[8px] uppercase tracking-[0.18em]" onClick={() => void connect()}>{account ? "Wallet connected" : "Connect Xaman"}</button><button disabled={!intent} className="border border-black bg-white px-5 py-4 text-[8px] uppercase tracking-[0.18em] text-black disabled:opacity-25" onClick={() => void sign()}>Open signing request</button></div></div></div>
        <aside className="flex flex-col bg-white p-6"><p className="text-[9px] uppercase tracking-[0.22em] text-black/40">Witness state</p><p className="mt-5 text-xl font-light leading-8">{status}</p><dl className="mt-8 space-y-4 border-t border-black/20 pt-5 font-mono text-[8px] uppercase tracking-[0.13em]"><div className="flex justify-between gap-4"><dt className="text-black/35">Source hash</dt><dd>{hash ? hash.slice(0, 12) : "Waiting"}</dd></div><div className="flex justify-between gap-4"><dt className="text-black/35">Intent</dt><dd>{intent ? "Prepared" : "Waiting"}</dd></div><div className="flex justify-between gap-4"><dt className="text-black/35">Wallet</dt><dd>{account ? `${account.slice(0, 8)}…` : "Disconnected"}</dd></div><div className="flex justify-between gap-4"><dt className="text-black/35">Payload</dt><dd>{payload?.uuid ? payload.uuid.slice(0, 8) : "Waiting"}</dd></div><div className="flex justify-between gap-4"><dt className="text-black/35">Transaction</dt><dd>{transaction ? `${transaction.slice(0, 10)}…` : "Waiting"}</dd></div><div className="flex justify-between gap-4"><dt className="text-black/35">Propagation</dt><dd>{propagation}</dd></div></dl>{payload?.next?.always ? <a className="mt-8 border border-black/30 p-4 text-center text-[8px] uppercase tracking-[0.18em]" href={payload.next.always} rel="noreferrer" target="_blank">Open in Xaman</a> : null}<div className="mt-auto grid gap-2 pt-10"><button disabled={!payload?.uuid} className="border border-black/25 p-4 text-[8px] uppercase tracking-[0.18em] disabled:opacity-25" onClick={() => void checkResult()}>Check result</button><button disabled={!transaction || propagation !== "idle"} className="border border-black p-4 text-[8px] uppercase tracking-[0.18em] disabled:opacity-25" onClick={() => void archive()}>{propagation === "idle" ? "Archive verified result" : "Propagation in progress"}</button></div></aside>
      </section>
      {intent ? <details className="mt-8 border border-black/20 p-5"><summary className="cursor-pointer text-[8px] uppercase tracking-[0.18em] text-black/40">Prepared transaction evidence</summary><pre className="mt-5 overflow-x-auto text-[8px] leading-5 text-black/45">{JSON.stringify(intent, null, 2)}</pre></details> : null}
    </div>
  </main>;
}
