  ...
  return <main className="min-h-screen bg-black text-white">
    ...
    <div className="mx-auto max-w-[1600px] px-5 py-10 md:px-8 md:py-16">
      ...
      <section className="mt-12 grid gap-px bg-white/20 lg:grid-cols-[1fr_330px]">
        <div className="space-y-10 bg-black p-6 md:p-8"><div><p className="text-[9px] uppercase tracking-[0.22em] text-white/40">01 / Select admitted FoldPortrait work</p><label className="mt-5 block border border-white/25 p-4 text-[8px] uppercase tracking-[0.16em] text-white/45">Canonical mint catalog<select className="mt-3 w-full bg-black text-sm normal-case text-white" value={preparedId} onChange={(event) => choosePrepared(event.target.value)}><option value="">Select sealed work</option>{batch.map((work) => <option key={work.artifact_id} value={work.artifact_id}>{work.sequence ? `${work.sequence}. ` : ""}{work.title}</option>)}</select></label><p className="mt-3 font-mono text-[8px] uppercase leading-5 tracking-[0.13em] text-white/30">New works enter here only after FoldPortrait archives and sequences them in its canonical catalog.</p></div>
