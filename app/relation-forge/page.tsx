      ...
      <div className="relation-grid border-b border-white/20">
        <section className="border-b border-white/20 p-5 md:border-r md:border-b-0 md:p-8">
          <button className="relation-drop grid w-full place-items-center border border-white/20 text-left" onClick={() => inputRef.current?.click()} type="button">
            ...
            </> : <span className="font-mono text-[10px] uppercase tracking-[.2em] text-white/45">Admit local image</span>}
          </button>
          <input ref={inputRef} accept="image/*" className="hidden" onChange={(event) => void ingest(event.target.files?.[0])} type="file" />
          <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 font-mono text-[9px] uppercase tracking-[.14em] text-white/34">
            <span>{status}</span>
        ...
        <section className="p-5 md:p-8">
          <p className="font-mono text-[9px] uppercase tracking-[.2em] text-white/42">01 / Confirm the witness</p>
          <label className="mt-8 block text-xs text-white/52">Visible evidence — one observable statement per line</label>
          ...
          <div className="mt-7 grid grid-cols-2 gap-6">
            <label className="text-xs text-white/52">Event
              <select className="relation-input mt-2" onChange={(event) => setEvent(event.target.value as EventState)} value={event}>
                {(["received", "opened", "placed", "prepared", "returned", "held"] as EventState[]).map((value) => <option className="bg-black" key={value}>{value}</option>)}
              </select>
            </label>
            <label className="text-xs text-white/52">Field term
              <select className="relation-input mt-2" onChange={(event) => setSystemTerm(event.target.value as SystemTerm)} value={systemTerm}>
                {grammar.systemTerms.map((value) => <option className="bg-black" key={value}>{value}</option>)}
              </select>
            </label>
          </div>

          <label className="mt-7 block text-xs text-white/52">Lived field — the relation the object enters</label>
          ...
          <label className="mt-8 flex cursor-pointer items-start gap-3 border-t border-white/16 pt-6 text-xs leading-5 text-white/58">
          ...
          <button className="mt-7 w-full border border-white bg-white px-5 py-4 font-mono text-[10px] uppercase tracking-[.2em] text-black disabled:border-white/20 disabled:bg-transparent disabled:text-white/25" disabled={!image || !confirmed || evidence.length === 0 || !livedField.trim()} onClick={compose} type="button">Compose witnessed relations</button>
        </section>
      </div>

      {couplets.length > 0 && <section className="grid border-b border-white/20 lg:grid-cols-[18rem_1fr]">
        <aside className="border-b border-white/20 p-5 lg:border-r lg:border-b-0 md:p-8">
          <p className="font-mono text-[9px] uppercase tracking-[.2em] text-white/42">02 / Select</p>
          <div className="mt-6 space-y-2">
            {couplets.map((couplet, index) => <button className={`w-full border px-4 py-3 text-left font-mono text-[9px] uppercase tracking-[.15em] ${selected === index ? "border-white text-white" : "border-white/16 text-white/38"}`} key={couplet.id} onClick={() => choose(index)} type="button">{String(index + 1).padStart(2, "0")} / {couplet.movement}</button>)}
          </div>
        </aside>
        <div className="p-5 md:p-8 lg:p-14">
          <p className="font-mono text-[9px] uppercase tracking-[.2em] text-white/42">03 / Human judgment</p>
          <div className="mt-9 max-w-4xl space-y-2">
            {editedLines.map((line, index) => <input aria-label={`Couplet line ${index + 1}`} className="relation-couplet w-full border-0 border-b border-white/14 bg-transparent py-3 text-white outline-none focus:border-white" key={index} onChange={(event) => setEditedLines((current) => index === 0 ? [event.target.value, current[1]] : [current[0], event.target.value])} value={line} />)}
          </div>
          <div className="mt-10 flex flex-wrap gap-2">
            {constraints.checks.map((check) => <span className={`border px-3 py-2 font-mono text-[8px] uppercase tracking-[.13em] ${check.passed ? "border-white/20 text-white/45" : "border-white text-white"}`} key={check.label}>{check.passed ? "✓" : "×"} {check.label}</span>)}
          </div>
          <div className="mt-9 flex flex-wrap gap-3">
            <button className="border border-white px-5 py-3 font-mono text-[9px] uppercase tracking-[.17em]" onClick={() => void navigator.clipboard.writeText(editedLines.join("\n"))} type="button">Copy couplet</button>
            <button className="border border-white/25 px-5 py-3 font-mono text-[9px] uppercase tracking-[.17em] text-white/58 disabled:opacity-30" disabled={!witness || !constraints.valid} onClick={() => witness && downloadJson(witness, `witness-couplet-${witness.composition.id}.json`)} type="button">Export witness</button>
          </div>
        </div>
      </section>}

      <footer className="grid gap-8 px-5 py-10 text-xs leading-5 text-white/42 md:grid-cols-3 md:px-8">
        <p><span className="text-white/68">Source fidelity.</span><br />Image bytes remain local and are never included in the exported witness.</p>
