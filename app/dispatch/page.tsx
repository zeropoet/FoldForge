...
export default function Dispatch() {
  ...
  const print = async () => {
    ...
    if (!bridgeToken) {
      setStatus("Start npm run dispatch:bridge locally to pair a print session");
      return;
    }
    ...
    try {
      ...
      const result = await response.json() as { error?: string; job_id?: string; jobs_accepted?: number };
      if (!response.ok) throw new Error(result.error || "Local printer bridge rejected the batch");
      setStatus(`${result.job_id || "Print job"} accepted / batch ${result.jobs_accepted || 1} / bridge remains paired`);
    } catch (error) {
