...
export default function TemporalForge() {
  ...
  return <main className="temporal-shell min-h-screen text-black">
    <PublicHeader active="temporal" />
    <div className="mx-auto max-w-[1600px] px-5 py-10 md:px-8 md:py-16">
      <section className="grid gap-10 border-b border-black/20 pb-12 lg:grid-cols-[1fr_0.75fr] lg:items-end">
        <div><p className="text-[9px] uppercase tracking-[0.25em] text-black/40">Collection frames / temporal recurrence</p><h1 className="mt-5 max-w-3xl text-5xl font-light tracking-[-0.055em] md:text-7xl">Temporal Forge</h1><p className="mt-7 max-w-2xl text-sm leading-7 text-black/55">Sequence a collection as frames, watch its visual grammar accumulate through time, and render the observation as a loop with a verifiable order.</p></div>
        <div className="border border-black/20 p-5 font-mono text-[8px] uppercase leading-5 tracking-[0.14em] text-black/35">Local by default<br />Natural filename order on ingest<br />GIF89a + MP4 / up to 1440 × 1440<br />Witnessed source sequence</div>
      </section>

      {!frames.length ? <button className="mt-12 grid min-h-[430px] w-full place-items-center border border-dashed border-black/30 px-8 text-center hover:border-black" onClick={() => inputRef.current?.click()}><span><span className="block text-xl font-light">Admit collection frames</span><span className="mt-4 block font-mono text-[8px] uppercase tracking-[0.2em] text-black/35">Choose multiple PNG, JPEG, WebP, GIF, or SVG files</span></span></button> : <>
        <section className="mt-12 grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="temporal-stage relative grid aspect-square max-h-[78vh] place-items-center overflow-hidden border border-black/20" style={{ backgroundColor: FRAME_BACKGROUNDS[frameBackground] }}>
            ...
            <div className="absolute inset-x-0 bottom-0 flex justify-between bg-white/75 p-4 font-mono text-[8px] uppercase tracking-[0.14em]"><span>{String(frameIndex + 1).padStart(4, "0")} / {String(frames.length).padStart(4, "0")}</span><span className="max-w-[60%] truncate text-black/45">{current?.name}</span></div>
          </div>
          <aside className="flex flex-col border border-black/20 p-5">
            <p className="text-[9px] uppercase tracking-[0.22em] text-black/40">Sequence state</p><p className="mt-4 text-3xl font-light">{duration.toFixed(2)} seconds</p><p className="mt-2 font-mono text-[8px] uppercase tracking-[0.15em] text-black/30">{frames.length} frames / {fps} fps / loop</p>
            <div className="mt-9 grid grid-cols-3 border border-black/25"><button className="p-4 text-sm" onClick={() => setFrameIndex((frameIndex - 1 + frames.length) % frames.length)} aria-label="Previous frame">←</button><button className="border-x border-black/25 p-4 text-[8px] uppercase tracking-[0.14em]" onClick={() => setPlaying((value) => !value)}>{playing ? "Pause" : "Play"}</button><button className="p-4 text-sm" onClick={() => setFrameIndex((frameIndex + 1) % frames.length)} aria-label="Next frame">→</button></div>
            <label className="mt-8 text-[8px] uppercase tracking-[0.18em] text-black/40">Cadence / {fps} fps<input className="mt-4 w-full" type="range" min="1" max="24" value={fps} onChange={(event) => setFps(Number(event.target.value))} /></label>
            <label className="mt-8 text-[8px] uppercase tracking-[0.18em] text-black/40">Frame / {frameIndex + 1}<input className="mt-4 w-full" type="range" min="0" max={frames.length - 1} value={frameIndex} onChange={(event) => { setPlaying(false); setFrameIndex(Number(event.target.value)); }} /></label>
            <fieldset className="mt-8">
              <legend className="text-[8px] uppercase tracking-[0.18em] text-black/40">Alpha background</legend>
              <div className="mt-4 grid grid-cols-2 border border-black/25">
            ...
            <fieldset className="mt-8">
              <legend className="text-[8px] uppercase tracking-[0.18em] text-black/40">Render size</legend>
              <div className="mt-4 grid grid-cols-3 border border-black/25">
            ...
            <button aria-pressed={inverted} className="temporal-inversion-option mt-8 flex items-center justify-between border border-black/25 px-4 py-3 text-[8px] uppercase tracking-[0.16em]" onClick={() => { const next = !inverted; setInverted(next); setStatus(`Color inversion ${next ? "enabled" : "disabled"}`); }} type="button"><span>Color inversion</span><span>{inverted ? "On" : "Off"}</span></button>
            <div className="mt-auto grid gap-2 pt-10"><button className="border border-black/40 px-5 py-4 text-[8px] uppercase tracking-[0.18em] disabled:opacity-35" disabled={exporting} onClick={() => void exportGif()}>{exporting ? "Rendering…" : "Render animated GIF"}</button><button className="border border-black/40 px-5 py-4 text-[8px] uppercase tracking-[0.18em] disabled:opacity-35" disabled={exporting} onClick={() => void exportMp4()} title="Render one sequence cycle as MP4">{exporting ? "Rendering…" : "Render MP4"}</button><button className="border border-black/20 px-5 py-4 text-[8px] uppercase tracking-[0.18em]" onClick={exportWitness}>Export sequence witness</button><button className="px-5 py-3 text-[8px] uppercase tracking-[0.18em] text-black/40" onClick={() => inputRef.current?.click()}>Replace frames</button></div>
          </aside>
        </section>
        <section className="mt-8"><div className="mb-4 flex justify-between font-mono text-[8px] uppercase tracking-[0.15em] text-black/35"><span>{status}</span><span>{sequenceId}</span></div><div className="temporal-strip flex gap-2 overflow-x-auto pb-3">{frames.map((frame, index) => <article aria-current={index === frameIndex} className="temporal-frame w-32 shrink-0 border border-black/15 p-2 opacity-55" key={frame.id}><button className="block w-full" onClick={() => { setPlaying(false); setFrameIndex(index); }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <span className="block aspect-square w-full" style={{ backgroundColor: FRAME_BACKGROUNDS[frameBackground] }}><img alt="" className="h-full w-full object-contain" src={frame.url} style={{ filter: inverted ? "invert(1)" : "none" }} /></span><span className="mt-2 block truncate text-left font-mono text-[7px] text-black/50">{String(index + 1).padStart(4, "0")} {frame.name}</span></button><div className="mt-2 grid grid-cols-2 gap-1"><button disabled={index === 0} className="border border-black/15 py-1 text-[8px] disabled:opacity-20" onClick={() => reorder(index, index - 1)}>←</button><button disabled={index === frames.length - 1} className="border border-black/15 py-1 text-[8px] disabled:opacity-20" onClick={() => reorder(index, index + 1)}>→</button></div></article>)}</div></section>
      </>}
